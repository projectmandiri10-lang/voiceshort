...
function stripLiteLlmGeminiPrefix(model: string): string {
  return model.startsWith("gemini/gemini-") ? model.slice("gemini/".length) : model;
}
...
export function findDefaultVoiceForGender(provider: AiProvider, gender: JobVoiceGender): TtsVoiceOption {
...
export function findTtsVoiceByName(voiceName: string, provider?: AiProvider): TtsVoiceOption | undefined {
  return ALL_TTS_VOICES.find(
    (voice) => voice.voiceName === voiceName && (!provider || voice.provider === provider)
  );
}

export function isKnownTtsVoiceName(voiceName: string, provider?: AiProvider): boolean {
