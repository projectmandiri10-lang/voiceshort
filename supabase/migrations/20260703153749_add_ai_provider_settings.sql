...
set
  ...
  script_fallback_provider = coalesce(nullif(script_fallback_provider, ''), 'zai'),
  tts_provider = coalesce(nullif(tts_provider, ''), 'zai'),
...
alter table public.app_settings
  ...
  alter column tts_provider set default 'zai',
...
alter table public.app_settings
  ...
  add constraint app_settings_script_provider_check
    check (script_provider in ('gemini_direct', 'zai')),
  add constraint app_settings_script_fallback_provider_check
    check (script_fallback_provider in ('gemini_direct', 'zai')),
  add constraint app_settings_tts_provider_check
    check (tts_provider in ('gemini_direct', 'zai')),
  add constraint app_settings_tts_fallback_provider_check
    check (tts_fallback_provider in ('gemini_direct', 'zai'));
