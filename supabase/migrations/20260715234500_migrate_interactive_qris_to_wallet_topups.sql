...
alter table public.payment_orders
  ...
  check (provider in ('webqris', 'interactive_qris'));

create unique index if not exists payment_orders_pending_interactive_qris_amount_uidx
  on public.payment_orders(total_amount_idr)
  where provider = 'interactive_qris' and status = 'pending';

alter table public.qris_webhook_events
